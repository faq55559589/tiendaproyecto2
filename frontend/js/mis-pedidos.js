document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        if (ordersNotice) {
            ...
            ordersNotice.className = 'alert alert-danger';
        }
        return;
    }
    ...
    if (ordersNotice) {
        ...
        ordersNotice.className = 'alert alert-light border';
    }
    ...
    ordersList.innerHTML = orders.map((order) => `
        <article class="card mb-3 border-0 shadow-sm">
            ...
            <div class="card-body">
                <p class="text-muted small mb-3">${new Date(order.createdAt).toLocaleString('es-UY')}</p>
                ${order.items.map((item) => `
                    <div class="d-flex align-items-center gap-3 mb-3">
                        ...
                        <strong class="text-danger">${GolazoStore.formatPrice(item.price * item.quantity)}</strong>
                    </div>
                `).join('')}
                <hr>
                <div class="row g-3">
                    ...
                    <div class="col-md-4 text-md-end">
                        ...
                        <div class="fs-5 fw-bold text-danger">${GolazoStore.formatPrice(order.total)}</div>
                    </div>
                </div>
            </div>
        </article>
